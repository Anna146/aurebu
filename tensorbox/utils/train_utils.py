...
def load_idl_tf(idlfile, H, phase, jitter):
    ...
    for anno in annolist:
        ...
        annos.append(anno)
    random.seed(0)
    if H['data']['truncate_data'] is not None and phase == 'train':
        annos = annos[:int(H['data']['truncate_data']*len(annos))]
        print(int(H['data']['truncate_data']*len(annos)))
...
def load_data_gen(H, phase, jitter):
    ...
    data = load_idl_tf(H["data"]['%s_idl' % phase], H, phase, jitter={'train': jitter, 'test': False}[phase])
...
def add_rectangles_plain(true_rects, H, orig_image, confidences, boxes, use_stitching=False, rnn_len=1, min_conf=0.1, show_removed=True, tau=0.25, show_suppressed=True, pred='pred'):
    ...
    for n in range(rnn_len):
        for y in range(H["grid_height"]):
            for x in range(H["grid_width"]):
                ...
                clas = np.argmax(confidences_r[0, y, x, n, 1:]) + 1
                all_rects[y][x].append(Rect(abs_cx,abs_cy,w,h,conf,clas*1.0))
    ...
    for rect in acc_rects:
        ...
        r.classID = int(rect.clas)
        rects.append(r)
    
    if H['iou'] != None:    
        rects = filter_close(rects,H['iou'])
    
    return image, rects

def add_rectangles(true_rects, H, orig_image, confidences, boxes, use_stitching=False, rnn_len=1, min_conf=0.1, show_removed=True, tau=0.25, show_suppressed=True, pred='pred'):
    image = np.copy(orig_image[0])
    num_cells = H["grid_height"] * H["grid_width"]
    boxes_r = np.reshape(boxes, (-1,
                                 H["grid_height"],
    ...
    pairs.append((acc_rects, color))
        
    with open('/home/anna/tensorbox/data/names/reference_dict.txt', 'r') as fil:
    ...
    for rect_set, color in pairs:
        for rect in rect_set:
            if rect.confidence > min_conf:
                #print(rect.clas)
                num = int(rect.clas)
                cv2.putText(image,refer_dict[num] if num in refer_dict else '', (rect.cx-3,rect.cy-3), cv2.FONT_HERSHEY_PLAIN, 1, color)
                cv2.rectangle(image,
    ...
    if len(true_rects) > 0:   
     for rect in true_rects:
            cv2.rectangle(image,
                    ...
                    (0, 0, 255),
                    1)
            num = int(round(rect.classID))
            cv2.putText(image,refer_dict[num] if num in refer_dict else '', (int(rect.x2),int(rect.y2)), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255))
...
def filter_close(rects, max_dis):
    res_rects = []
    rem = []
    for i in range(len(rects)-1):
        r1 = rects[i]
        diag1 = dista(r1.x1,r1.y1,r1.x2,r1.y2) / 2.0
        c1 = (r1.x1 + (r1.x2-r1.x1) / 2., r1.y1 + (r1.y2-r1.y1) / 2.)
        for j in range(i+1,len(rects)):
          r2 = rects[j]
          if r1.classID != r2.classID:
             continue
          r2 = rects[j]
          diag2 = dista(r2.x1,r2.y1,r2.x2,r2.y2) / 2.0
          c2 = (r2.x1 + (r2.x2-r2.x1) / 2., r2.y1 + (r2.y2-r2.y1) / 2.)
          pdist = dista(c1[0],c1[1],c2[0],c2[1])
          if pdist / (diag1 + diag2) < max_dis:
              rem += [j] if r2.score < r1.score else [i]
...
def add_rectangles_big(true_rects, H, orig_image, confidences, boxes, sup = None, use_stitching=False, rnn_len=1, min_conf=0.1, show_removed=True, tau=0.25, show_suppressed=True, pred='pred'):
    image = np.copy(orig_image[0])
    num_cells = H["grid_height"] * H["grid_width"]
    boxes_r = np.reshape(boxes, (-1,
                                 H["grid_height"],
    ...
    all_rects_r = [r for row in all_rects for cell in row for r in cell]
    if sup != None:
      acc_rects = [x[1] for x in enumerate(all_rects_r) if x[0] in sup]
    else:
      if use_stitching:
        from stitch_wrapper import stitch_rects
        acc_rects = stitch_rects(all_rects, tau)
    ...
    for rect in acc_rects:
        ...
        rects.append(r)
    '''    
    rects = filter_close(rects,2.0)
    '''
    pred_anno = al.Annotation()
    pred_anno.rects = rects
    pred_anno = rescale_boxes((H["image_height"], H["image_width"]), pred_anno, image.shape[0], image.shape[1])
    
    with open('/home/anna/tensorbox/data/names/reference_dict.txt', 'r') as fil:
        reader = csv.reader(fil) 
        refer_dict = {int(rows[1]):rows[0] for rows in reader}
        for rect in pred_anno:
          if rect.score > min_conf:
            cv2.rectangle(image,
                    (int(rect.x1), int(rect.y1)),
                    (int(rect.x2), int(rect.y2)),
                    (0, 255, 0),
            ...
            num = int(round(rect.classID)) 
            conf = rect.score
            cv2.putText(image,refer_dict[num] if num in refer_dict else '', (int(rect.x1),int(rect.y1)-3), cv2.FONT_HERSHEY_PLAIN, 3, (0, 255, 0), thickness = 4)
            #cv2.putText(image,str(conf), (int(rect.x1) -3,int(rect.y2)+5), cv2.FONT_HERSHEY_PLAIN, 2, (255, 0, 0), thickness = 4)
    ...
    if len(true_rects) > 0:   
     for rect in true_rects:
            cv2.rectangle(image,
                    ...
                    3)
            num = int(round(rect.classID)) 
            cv2.putText(image,refer_dict[num] if num in refer_dict else '', (int(rect.x2),int(rect.y2)+3), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255), thickness = 4)
    '''
    return image, rects
