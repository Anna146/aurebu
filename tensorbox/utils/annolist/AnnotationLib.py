...
class AnnoRect(object):
    ...
    def writeJSON(self):
        jdoc = {"x1": self.x1, "x2": self.x2, "y1": self.y1, "y2": self.y2, 'label':self.classID}
    ...
    #def isMatchingPascal(self, other, minOverlap):               
    def isMatchingIgnore(self, other, minOverlap):
        overlap = self.overlap_pascal(other)
    ...
    #def isMatchingPascal(self, other, minOverlap):       
    def isMatchingLocalizeClass(self, other, minOverlap):
        overlap = self.overlap_pascal(other)
        maxOverlap = minOverlap
        minOverlap = 0.1
        if (overlap >= minOverlap and overlap < maxOverlap and (self.classID == -1 or other.classID == -1 or self.classID == other.classID)):
            return 1
        else:
            return 0
            
    def isMatchingLocalizeIgnore(self, other, minOverlap):
        overlap = self.overlap_pascal(other)
        maxOverlap = minOverlap
        minOverlap = 0.1
        if (overlap >= minOverlap and overlap < maxOverlap):
            return 1
        else:
            return 0
