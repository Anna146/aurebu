    ...
    def match(self, style, minCover, minOverlap, maxDistance):
        for i in xrange(self.n):
            ...
            for j in xrange(self.m):
                ...
                # Class ignorant
                if(style == -1):
                    if (detRect.isMatchingIgnore(annoRect, minOverlap)):
                        self.M[i].append(self.n + 1 + j)
...
# MA: full version
def comp_prec_recall_all_params(annoIDL, detIDL, ignoreIDL, lambda0 = 1.0, lambda1 = 0.1, prec_file = 'someprecision.txt', minWidth=0, minHeight=0, maxWidth=float('inf'), maxHeight=float('inf'),
                                matchingStyle=1, minCover=0.5, minOverlap=0.5, maxDistance=0.5, ignoreOverlap=0.9, verbose=False, clas = -1):
    ...
    if noAnnotations == 0 or P == 0:
         print('DIVISION BY 0!!!')
         print(P)
         print(noAnnotations)
         return precs, recalls, scores, fppi, graphs
        
    with open(prec_file, "a") as filpr:
        filpr.write(' '.join([str(lambda0), str(lambda1), str(TP), str(P), str(noAnnotations), str(TP*1.0 / noAnnotations), str(TP*1.0 / P), str(clas)]) + '\n')
...
def main():
    ...
    parser.add_option("--ignore_classes", action="store_true", dest="classIgnore")
    ...
    parser.add_option("--lambda1", action="store", type="float", dest="lambda1")
    parser.add_option("--lambda0", action="store", type="float", dest="lambda0")
    parser.add_option("--prec_file", action="store", type="string", dest="prec_file")
    ...
    print(options.classID)
    if(options.classID != -1):
        #for anno in annoIDL:
            #anno.rects = [rect for rect in anno.rects if (int(rect.classID) == options.classID  or rect.classID == -1 or options.classID == -1)]            
        for anno in detIDL:
            anno.rects = [rect for rect in anno.rects if (int(rect.classID) == options.classID or rect.classID == -1 or options.classID == -1)]
        for anno in ignoreIDL:
            anno.rects = [rect for rect in anno.rects if (int(rect.classID) == options.classID or rect.classID == -1)]
    ...
    verbose = True;
    print(len(annoIDL))
    precs, recalls, scores, fppi, graphs = comp_prec_recall_all_params(annoIDL, detIDL, ignoreIDL, options.lambda0, options.lambda1, options.prec_file,
                                                               ...
                                                               verbose=verbose, clas = options.classID);
