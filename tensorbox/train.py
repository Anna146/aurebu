...
def build_forward_backward(H, x, phase, boxes, flags):
    ...
    with tf.variable_scope('decoder', reuse={'train': None, 'test': True}[phase]):
        ...
        npd = np.array([[[0.,0.,0.,0.,1.]]]*outer_size)
        tfd = tf.to_float(tf.Variable(npd))
        
        true_classes = tf.reshape(tf.slice(tf.cast(perm_truth, 'int64'), [0,0,4], [outer_size,1,1]),[outer_size * H['rnn_len']])
        ...
        npc = np.array([[[1.,1.,1.,1.,0.]]]*outer_size)
        tfc = tf.to_float(tf.Variable(npc))
        
        
        residual = tf.reshape((perm_truth - pred_boxes * pred_mask)*tfc, [outer_size, H['rnn_len'], 5])#+tf.minimum((perm_truth - pred_boxes * pred_mask)*tfd,tfd)*tf.to_float(tf.Variable(np.array([[[0.,0.,0.,0.,10.]]]*outer_size))),
                              #
        ...
        if H['use_rezoom']:
            ...
            loss = confidences_loss + boxes_loss# + labels_loss

    return pred_boxes, pred_confidences, loss, confidences_loss, boxes_loss#, labels_loss
...
def build(H, q):
    ...
    loss, accuracy, confidences_loss, boxes_loss, labels_loss = {}, {}, {}, {}, {}
    #print('6')
    for phase in ['train', 'test']:
        ...
        pred_boxes_r = tf.reshape(pred_boxes, [H['batch_size'], grid_size, H['rnn_len'], 5])
        ...
        if phase == 'train':
            ...
            for p in ['train', 'test']:
                ...
                #tf.summary.scalar("%s/labels_loss" % p, labels_loss[p])
                #tf.summary.scalar("%s/labels_loss/smooth" % p,
                    #moving_avg.average(labels_loss[p]))
        ...
        if phase == 'test':
            ...
            def log_image(np_img, np_confidences, np_boxes, np_global_step, pred_or_true):
                ...
                merged = train_utils.add_rectangles(dict(), H, np_img, np_confidences, np_boxes,
                                                    use_stitching=True,
                                                    rnn_len=H['rnn_len'])[0]
...
def train(H, test_images):
    ...
    with tf.Session(config=config) as sess:
        ...
        start = time.time()
        max_iter = H['solver'].get('max_iter', 1000000)
        for i in xrange(max_iter):
            ...
            if i % display_iter != 0:
                ...
                start = time.time()
                (train_loss, test_loss, test_accuracy, summary_str,
                    _, _) = sess.run([loss['train'], loss['test'], accuracy['test'],
                                      summary_op, train_op, smooth_op,
                                     ], feed_dict=lr_feed)
                if i % 30000 != 0 and H['data']['truncate_data'] != None:
                    with open("loss_decrease.txt", "a") as filpr:
                        filpr.write(' '.join([str(train_loss), str(test_loss), str(H['data']['truncate_data'])]) + '\n')
...
def main():
    ...
    parser.add_argument('--lambda0', default=None, type=float)
    parser.add_argument('--lambda1', default=None, type=float)
    parser.add_argument('--truncate', default=None, type=float)
    ...
    #H['save_dir'] = args.logdir + '/%s' % (args.hypes.split('/')[-1].split('.')[0])
    ...
    if args.lambda0 is not None:
        #H['solver']['head_weights'][0] = args.lambda0
        H['save_dir'] = args.logdir + '/%s_%s' % (str(args.lambda0),str(args.lambda1))
    if args.lambda0 is not None:
        H['solver']['head_weights'][1] = args.lambda1
    H['data']['truncate_data'] = args.truncate
    print(H['data']['truncate_data'])
    #print('3')
    train(H, test_images=[])
